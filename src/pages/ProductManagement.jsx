...
export default function ProductManagement() {
  ...
  const openModal = (prod = null) => {
    if (prod) {
      ...
      setCustomMaterial(prod.customMaterial || ""); 
  ...
  const handleQuickProduce = async (e) => {
    ...
    const newJobPayload = {
      ...
      product: {
        id: activeProduceProduct.id, name: activeProduceProduct.name, sku: activeProduceProduct.sku || "",
    ...
    } catch (error) { alert("Failed to generate job: " + error.message); } 
                ...
                filteredProducts.map((prod) => (
                  <tr key={prod.id} className="hover:bg-gray-800/30 transition-colors">
                    ...
                    <td className="py-4 px-6 text-sm text-gray-400">
                      ...
                      {prod.customMaterial && <div className="text-[10px] text-primary-400 mt-1 uppercase">Extras: {prod.customMaterial}</div>}
                    </td>
                    <td className="py-4 px-6">
                          ...
                          prod.default_sequence.map((step, i) => {
                            ...
                            return (
                              <div key={i} className="flex items-center gap-2">
                                ...
                                {mach && <span className="text-[10px] text-gray-500 font-mono truncate max-w-[120px]">👉 {mach.name}</span>}
                              </div>
                            );
                          })
                        ) : (
      ...
      {isProduceModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-primary-500/30 rounded-xl w-full max-w-md p-6 shadow-2xl shadow-primary-500/10">
            ...
            <p className="text-sm text-gray-400 mb-6">Instantly push <strong className="text-white">{activeProduceProduct?.name}</strong> to the factory floor using its locked machine routing.</p>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-8">
              ...
              <div className="bg-gray-950/50 p-5 rounded-lg border border-gray-800">
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                  <div><label className={labelClass}>Final Size</label><input type="text" value={size} onChange={e => setSize(e.target.value)} className={inputClass} placeholder="L x W x H" /></div>
                  <div><label className={labelClass}>Raw Sheet</label><input type="text" value={sheetSize} onChange={e => setSheetSize(e.target.value)} className={inputClass} placeholder="e.g., 25 x 36 in" /></div>
                  <div><label className={labelClass}>Material</label><input type="text" value={paperType} onChange={e => setPaperType(e.target.value)} className={inputClass} placeholder="e.g., Duplex" /></div>
                  <div><label className={labelClass}>GSM</label><input type="text" value={paperGsm} onChange={e => setPaperGsm(e.target.value)} className={inputClass} placeholder="e.g., 350 GSM" /></div>
                  <div><label className={labelClass}>Custom / Extras</label><input type="text" value={customMaterial} onChange={e => setCustomMaterial(e.target.value)} className={`${inputClass} border-primary-500/30`} placeholder="Any extra spec" /></div>
                </div>
              </div>
